...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                let aorusCfg = UIImage.SymbolConfiguration(pointSize: dateFont.pointSize, weight: .medium)\n"
        ...
        "                // SF Symbol images have ~30-40% invisible padding around the glyph.\n"
        "                // To make the VISIBLE glyph equal to digit height (capHeight), we must\n"
        "                // rasterise the image larger so the padded-down glyph hits capHeight.\n"
        "                // Vertical POSITION uses capH metrics (keeps icon centred on digits);\n"
        "                // image size is scaled independently so the glyph fills that zone.\n"
        "                let aorusCapH = dateFont.capHeight.rounded()\n"
        "                let aorusNatH = max(aorusTinted.size.height, 0.01)\n"
        "                let aorusNatW = aorusTinted.size.width\n"
        "                // Scale image so its height = capH / 0.68 ≈ capH * 1.47,\n"
        "                // compensating for SF Symbol padding so visible glyph ≈ capH.\n"
        "                let aorusTargetH = (aorusCapH / 0.68).rounded()\n"
        "                let aorusScale = aorusTargetH / aorusNatH\n"
        "                let aorusTargetSize = CGSize(width: max(1.0, (aorusNatW * aorusScale).rounded()), height: aorusTargetH)\n"
        "                let aorusRenderer = UIGraphicsImageRenderer(size: aorusTargetSize)\n"
        "                let aorusImg = aorusRenderer.image { _ in aorusTinted.draw(in: CGRect(origin: CGPoint.zero, size: aorusTargetSize)) }.withRenderingMode(.alwaysOriginal)\n"
        "                // CTRunDelegate metrics control vertical position only (not clip).\n"
        "                // ascent=capH+2, descent=0 → attachment centre at -capH/2 = digit centre.\n"
        "                // TextNode draws image centred at that point, so the larger image extends\n"
        "                // symmetrically above/below but the glyph sits exactly on the digits.\n"
