...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                // Drive the inline glyph metrics from the date font + the same\n"
        "                // UIScreenPixel offset the native eye/views icon uses, so the icon\n"
        "                // ends up centred on the line at exactly the eye icon's height.\n"
        "                // (TextNode draws the attachment at midY=(descent-ascent)/2 + 1.0; the\n"
        "                // eye icon sits at line-centre + 1.0 + (-UIScreenPixel) — matching both.)\n"
        "                let aorusMetrics = UnsafeMutablePointer<CGFloat>.allocate(capacity: 3)\n"
        "                aorusMetrics.initialize(repeating: 0.0, count: 3)\n"
        "                aorusMetrics[0] = dateFont.ascender + UIScreenPixel\n"
        "                aorusMetrics[1] = -dateFont.descender - UIScreenPixel\n"
