...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                // Render every icon at exactly the height of the time DIGITS (the font's\n"
        "                // cap height), aspect preserved, so trash and pencil are the same size AND\n"
        "                // the same size as the time text.\n"
        "                let aorusTargetH = dateFont.capHeight.rounded()\n"
        ...
        "                // Sit the icon exactly on the digits: TextNode draws the attachment centred\n"
        "                // at (descent-ascent)/2 + 1.0 from the baseline. With descent=0 and\n"
        "                // ascent=height+2, the centre lands at -height/2 — i.e. the icon's bottom is\n"
        "                // on the baseline and its top is at the cap height, matching the time digits.\n"
        "                let aorusMetrics = UnsafeMutablePointer<CGFloat>.allocate(capacity: 3)\n"
        "                aorusMetrics.initialize(repeating: 0.0, count: 3)\n"
        "                aorusMetrics[0] = aorusTargetH + 2.0\n"
        "                aorusMetrics[1] = 0.0\n"
