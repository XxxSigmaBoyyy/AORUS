...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                // Normalise every status icon to ONE target height (aspect preserved) so\n"
        "                // the trash and pencil glyphs render at exactly the same size — SF Symbols\n"
        "                // have different bounding boxes at the same point size (trash is taller).\n"
        "                let aorusTargetH = (dateFont.pointSize - 1.0).rounded()\n"
        "                let aorusScale = aorusTargetH / max(aorusTinted.size.height, 0.01)\n"
        "                let aorusTargetSize = CGSize(width: max(1.0, (aorusTinted.size.width * aorusScale).rounded()), height: aorusTargetH)\n"
        ...
        "                // Vertically centre the icon on the time digits (cap height) so it lines up\n"
        "                // with the time. TextNode draws the attachment centred at\n"
        "                // (descent-ascent)/2 + 1.0 from the baseline; the digits are centred at\n"
        "                // capHeight/2 above the baseline. Glyph box height = icon height (<= line\n"
        "                // height) so the status line never grows.\n"
        "                let aorusMetrics = UnsafeMutablePointer<CGFloat>.allocate(capacity: 3)\n"
        "                aorusMetrics.initialize(repeating: 0.0, count: 3)\n"
        "                aorusMetrics[0] = (aorusTargetH + dateFont.capHeight + 2.0) / 2.0\n"
        "                aorusMetrics[1] = max(0.0, (aorusTargetH - dateFont.capHeight - 2.0) / 2.0)\n"
