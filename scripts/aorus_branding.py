...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                // SF Symbol at dateFont.pointSize: UIKit guarantees the GLYPH height\n"
        "                // equals the font's cap height — exactly matching digit height.\n"
        "                // No manual rasterisation or magic scale factors needed.\n"
        ...
        "                let aorusImg = aorusSym.withTintColor(dateColor, renderingMode: .alwaysOriginal)\n"
        "                // CTRunDelegate: ascent=capH+2, descent=0 centres the image at\n"
        "                // capH/2 above baseline, matching the digit centroid exactly.\n"
        "                // TextNode draws the image centred at the frame midY — the image\n"
        "                // may overflow the metric box above/below, but the glyph stays\n"
        "                // correctly aligned because UIKit designs the symbol to sit on\n"
        "                // the baseline with top at capHeight (proven from SF Symbol spec).\n"
