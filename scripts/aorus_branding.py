...
def patch_view_once_direct_save_button(tg: Path) -> None:
    ...
    new_methods = (
        ...
        "                if success {\n"
        "                    DispatchQueue.main.async { self?._aorusShowSavedToast(isVideo: isVideo) }\n"
        "                }\n"
        "            }\n"
        "        }\n"
        "        if #available(iOS 14, *) {\n"
        ...
        "    }\n"
        "\n"
        "    @objc private func _aorusShowSavedToast(isVideo: Bool) {\n"
        '        let _agRu = UserDefaults.standard.string(forKey: "aorusgram_lang") == "ru"\n'
        "        let text: String\n"
        "        if _agRu {\n"
        '            text = isVideo ? "Видео сохранено в галерею" : "Фото сохранено в галерею"\n'
        "        } else {\n"
        '            text = isVideo ? "Video saved to gallery" : "Photo saved to gallery"\n'
        "        }\n"
        "        let container = UIView()\n"
        "        container.translatesAutoresizingMaskIntoConstraints = false\n"
        "        container.isUserInteractionEnabled = false\n"
        "        container.alpha = 0.0\n"
        "        container.transform = CGAffineTransform(translationX: 0, y: 16)\n"
        ...
        "        blur.layer.borderColor = UIColor.systemGreen.withAlphaComponent(0.45).cgColor\n"
        ...
        "            container.bottomAnchor.constraint(equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -84),\n"
        "        ])\n"
