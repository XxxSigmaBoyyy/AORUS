...
def patch_status_edit_delete_icons(tg: Path) -> None:
    ...
    new_date = (
        ...
        "                // Icon height MUST be a whole point: a fractional height (e.g. 8.5pt)\n"
        "                // maps to 25.5px at @3x, and the renderer's pixel rounding leaves a\n"
        "                // sub-pixel gap that makes the glyph look dropped below the digits.\n"
        "                // capHeight rounds to 8pt; +1 gives a crisp 9pt (27px @3x), one whole\n"
        "                // point taller than the digits, with its bottom still on the baseline.\n"
        "                let aorusCapH = dateFont.capHeight.rounded() + 1.0\n"
        ...
        "                let aorusImg = UIGraphicsImageRenderer(size: aorusFinalSize).image { _ in aorusGlyph.draw(in: CGRect(origin: CGPoint.zero, size: aorusFinalSize)) }.withRenderingMode(.alwaysOriginal)\n"
        "                // Position: image height == capH; with ascent=capH+2, descent=0 the\n"
        "                // image spans baseline..capHeight — exactly where the digits sit.\n"
